...
- **2026-03-09**: Fix Krisp noise suppression passthrough bug, bump v0.4.0-beta.21. ROOT CAUSE: `@livekit/krisp-noise-filter@0.4.1` — `createSharedBuffers()` zero-initializes `atomicState` Int32Array, so `atomicState[1]` (SET_NOISE_SUPPRESSION_LEVEL) starts at 0. The Worker reads this on every audio frame via `updateNoiseSuppressionLevelFromSharedArrayBufferState()`, overriding the constructor default of 100 with 0. Level 0 = no noise suppression (passthrough). FIX: Patch 5→3 in `scripts/patch-krisp.js` wraps the `new Int32Array(new SharedArrayBuffer(...))` in an IIFE that sets `[1]=100`. Also cleaned up diagnostic logging patches (3, 4) from `patch-krisp.js` and removed verbose debug instrumentation from `src/lib/noiseProcessor.ts`. Files: `scripts/patch-krisp.js`, `src/lib/noiseProcessor.ts`, `package.json`, `src-tauri/tauri.conf.json`.
- **2026-03-09**: Krisp cleanup — remove dead noise suppression artifacts. Removed 2 dead npm dependencies: `@sapphi-red/web-noise-suppressor` (was for RNNoise/Speex) and `deepfilternet3-noise-filter` (was for DeepFilter) — neither imported anywhere since the multi-model system was replaced by Krisp-only. Deleted `public/dtln/` (DTLN WASM model files, unused). Removed diagnostic `krisp-test.html` and `test-krisp.html` pages and the `krisp-test` entry from `vite.config.ts` rollupOptions. Created `docs/krisp-noise-suppression.md` documenting the full Krisp integration: 3-thread architecture, postinstall patches, SharedArrayBuffer bug root cause, file map, and troubleshooting guide. Files: `package.json`, `vite.config.ts`, `docs/krisp-noise-suppression.md` (new). Deleted: `public/dtln/`, `krisp-test.html`, `test-krisp.html`.
